...
impl SendTraffic {
    ...
    ///
    /// This is useful to handle a [`ReceiveTrafficState::ServiceSender`] event, but
...
pub struct ReceiveTraffic<Side: SideData> {
    ...
    pub(crate) pending_service_sender: bool,
}
...
impl<Side: SideData> ReceiveTraffic<Side> {
    ...
    pub fn read<'a>(
            ...
            match recv.process_recv_traffic::<Side>(received_tls, &mut state, &mut send_adapter) {
                ...
                Err(err) => {
                    return Err(ErrorWithAlert::new(
                        err,
                        send_adapter.as_locked().deref_mut(),
                    ));
                }
            };
        ...
        if let SendAdapter::Locked(_) = send_adapter {
            pending_service_sender = true;
        }
        ...
        if core::mem::take(&mut rt.pending_service_sender) {
            return Ok(ReceiveTrafficState::ServiceSender(ServiceSender { rt }));
        }
...
///        │            ╰───────────────────────╯
///        │             │      │       │      │
///        │    ReadMore │      │       │      │ CloseNotify
///        │             │      │       │      ▼
///        │             │      │       │   ╭──────────╮
///        ├─────────────╯      │       │   │ terminal │
///        │                    │       │   ╰──────────╯
///        │      ServiceSender │       │ Available
///        │    .into_next() -> │       │ .into_next() ->
...
///   you have more input.
/// - [`ServiceSender`]: receiving may have produced data to send. Call
///   [`ServiceSender::into_next()`] to resume receiving.
...
/// [`ServiceSender`]: ReceiveTrafficState::ServiceSender
...
pub enum ReceiveTrafficState<'a, Side: SideData> {
    ...
    ServiceSender(ServiceSender<Side>),
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ReadMore(_) => f
                ...
                .finish_non_exhaustive(),
            Self::ServiceSender(_) => f
                .debug_tuple("ServiceSender")
                .finish_non_exhaustive(),
            Self::Available(_) => f
...
impl<Side: SideData> ReceivedApplicationData<'_, Side> {
    ...
    ///
    /// Returns the next [`ReceiveTraffic`] state.
...
pub struct ServiceSender<Side: SideData> {
    rt: ReceiveTraffic<Side>,
}

impl<Side: SideData> ServiceSender<Side> {
...
impl SendOutput for SendAdapter<'_> {
    ...
    fn send_alert(&mut self, level: crate::msgs::AlertLevel, desc: AlertDescription) {
